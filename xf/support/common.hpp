...
struct static_cast_t {
    template <class From>
    constexpr To operator()(From &&f) const {return static_cast<To>(std::forward<From>(f));}
};

template <class To>
static constexpr auto static_cast_c = static_cast_t<To>();
...
struct construct_t {
    template <class ...Args>
    constexpr To operator()(Args &&...args) const {return To{std::forward<Args>(args)...};}
};

template <class To>
static constexpr auto construct_c = construct_t<To>();
...
template <class T> using type_in = typename T::type;
