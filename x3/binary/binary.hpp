...
namespace x3 {
    ...
    struct binary_lit_parser : parser<binary_lit_parser<V, T, endian, bits> > {
        ...
        bool parse(Iterator& first, Iterator const& last , Context& context, unused_type, Attribute& attr_param) const {
    ...
    struct any_binary_parser : parser<any_binary_parser<T, endian, bits > > {
        ...
        template <typename V>
        binary_lit_parser< V, T, endian, bits> operator()(V n) const {return {n}; }
    };
...
#undef BOOST_SPIRIT_MAKE_BINARY_PRIMITIVE
    ...
    struct get_info<any_binary_parser<T, endian::order::little, bits>> {
        ...
        std::string operator()(any_binary_parser<T, endian::order::little, bits> const& p) const {return "little-endian binary"; }
    };

    template <typename T, std::size_t bits>
    struct get_info<any_binary_parser<T, endian::order::big, bits>> {
        typedef std::string result_type;
        std::string operator()(any_binary_parser<T, endian::order::big, bits> const& p) const {return "big-endian binary"; }
    };

    template <typename V, typename T, std::size_t bits>
    struct get_info<binary_lit_parser<V, T, endian::order::little, bits>> {
        typedef std::string result_type;
        std::string operator()(binary_lit_parser<V, T, endian::order::little, bits> const& p) const {return "little-endian binary"; }
    };

    template <typename V, typename T, std::size_t bits>
    struct get_info<binary_lit_parser<V, T, endian::order::big, bits>> {
        typedef std::string result_type;
        std::string operator()(binary_lit_parser<V, T, endian::order::big, bits> const& p) const {return "big-endian binary"; }
    };
