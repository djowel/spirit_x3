...
#include <x3/operator/kleene.hpp>

namespace x3 { namespace detail {
    template <typename T> // handles repeat(exact)[p]
    struct exact_count {
        typedef T type;
    ...
    template <typename T> // handles repeat(min, max)[p]
    struct finite_count {
        typedef T type;
    ...
    template <typename T> // handles repeat(min, inf)[p]
    struct infinite_count {
        typedef T type;
...
namespace x3 {
    ...
    struct repeat_directive : unary_parser<Subject, repeat_directive<Subject,RepeatCountLimit>> {
        ...
        repeat_directive(Subject const& subject, RepeatCountLimit const& repeat_limit_) : base_type(subject) , repeat_limit(repeat_limit_) {}
        ...
        bool parse(Iterator& first, Iterator const& last , Context const& context, RContext& rcontext, Attribute& attr) const {
            ...
            for (/**/; !repeat_limit.got_min(i); ++i) {
                if (!detail::parse_into_container(this->subject, local_iterator, last, context, rcontext, attr)) return false;
            }

            first = local_iterator;
            // parse some more up to the maximum specified
            for (/**/; !repeat_limit.got_max(i); ++i) {
                if (!detail::parse_into_container(this->subject, first, last, context, rcontext, attr)) break;
            }
            return true;
        }
    ...
    struct repeat_gen {
        ...
        struct repeat_gen_lvl1 {
            ...
            operator[](Subject const& subject) const {return { as_parser(subject),repeat_limit }; }
        ...
        operator()(T const exact) const {return { detail::exact_count<T>{exact} }; }

        template <typename T>
        repeat_gen_lvl1<detail::finite_count<T>>
        operator()(T const min_val, T const max_val) const {return { detail::finite_count<T>{min_val,max_val} }; }

        template <typename T>
        repeat_gen_lvl1<detail::infinite_count<T>> operator()(T const min_val, inf_type const &) const {return { detail::infinite_count<T>{min_val} }; }
    };
