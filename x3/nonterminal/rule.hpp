...
#include <x3/nonterminal/detail/rule.hpp>
...
namespace x3 {
    ...
    template <typename ID, typename Attribute, typename Iterator , typename Context, typename ActualAttribute, std::enable_if_t<!has_custom_definition<ID>::value, int>>
    inline detail::default_parse_rule_result parse_rule(rule<ID, Attribute> rule_ , Iterator& first, Iterator const& last , Context const& context, ActualAttribute& attr) {
        static_assert(!is_same<decltype(get<ID>(context)), unused_type>::value, "BOOST_SPIRIT_DEFINE undefined for this rule.");
    ...
    struct rule_definition : parser<rule_definition<ID, RHS, Attribute, force_attribute_>> {
        ...
        bool parse(Iterator& first, Iterator const& last , Context const& context, unused_type, Attribute_& attr) const {
            return detail::rule_parser<attribute_type, ID> ::call_rule_definition(rhs, name, first, last , context , attr , mpl::bool_<force_attribute>());
        }
    ...
    struct rule : parser<rule<ID, Attribute>> {
        ...
        rule_definition< ID, typename extension::as_parser<RHS>::value_type, Attribute, force_attribute_>
        operator=(RHS const& rhs) const {return { as_parser(rhs), name }; }

        template <typename RHS>
        rule_definition< ID, typename extension::as_parser<RHS>::value_type, Attribute, true>
        operator%=(RHS const& rhs) const {return { as_parser(rhs), name }; }
    ...
    struct get_info<T, typename enable_if<traits::is_rule<T>>::type> {
        ...
        std::string operator()(T const& r) const {return r.name; }
    };
