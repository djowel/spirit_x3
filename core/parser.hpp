...
namespace x3 {
    ...
    struct binary_parser : parser<Derived> {
        ...
        binary_parser(Left const& left, Right const& right) : left(left), right(right) {}
    ...
    ///////////////////////////////////////////////////////////////////////////
    namespace extension {namespace detail {namespace as_parser_guard {
                ...
                struct deduce_as_parser {
                    typedef R type;
                    typedef typename boost::remove_cv< typename boost::remove_reference<R>::type >::type value_type;
    ...
    as_parser(T const& x) {return extension::as_parser<T>::call(x); }

    template <typename Derived>
    inline Derived const&
    as_parser(parser<Derived> const& p) {return p.derived(); }
    ...
    std::string what(Parser const& p) {return get_info<Parser>()(p); }
}
...
namespace x3 { namespace traits {
    ...
    struct has_attribute<x3::binary_parser<Left, Right, Derived>, Context> : mpl::bool_<has_attribute<Left, Context>::value || has_attribute<Right, Context>::value> {};
}}
