...
constexpr auto radix_mask(R radix) {
    constexpr auto is = hana::reverse(indices_c<std::numeric_limits<T>::digits>);
...
constexpr auto radix_remainder(R radix, S start) {
    constexpr auto is = indices_c<std::numeric_limits<T>::digits>;
...
struct uint_parser : expression_base {
    ...
    auto check(Window &w) const {return digit_range(w, hana::size_c<std::numeric_limits<T>::digits>);}
    ...
    template <class Data> T parse(Data data) const {
        ...
        for (auto it = data[0_c]; it != data[1_c]; ++it) std::cout << "[" << *it << "]" << std::endl;
...
#define BOOST_SPIRIT_X3_UINT_PARSER(uint_type, name)                            \
    ...
    BOOST_SPIRIT_X3_UINT_PARSER(unsigned long long, ulong_long_x)
