...
namespace x3 {
    ...
    struct skip_directive : unary_parser<Subject, skip_directive<Subject, Skipper>> {
        ...
        skip_directive(Subject const& subject, Skipper const& skipper) : base_type(subject) , skipper(skipper) {}
    ...
    struct reskip_gen {
        ...
        struct skip_gen {
            ...
            operator[](Subject const& subject) const {return { as_parser(subject), skipper_ }; }

            Skipper skipper_;
        };

        template <typename Skipper>
        skip_gen<Skipper> const operator()(Skipper const& skipper) const {return { skipper }; }
